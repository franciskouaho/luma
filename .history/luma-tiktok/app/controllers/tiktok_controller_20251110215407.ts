...
export default class TikTokController {
  public async redirect({ ally }: HttpContext) {
    return ally.use('tiktok').redirect()
  }

  public async callback({ response, ally, request }: HttpContext) {
    const tiktok = ally.use('tiktok')
    ...
    if (tiktok.hasError()) {
      return this.redirectToApp(response, null, 'Erreur lors de la connexion TikTok')
    }

    const user = await tiktok.user()
    const tokens = tiktok.tokens
  ...
  public async consumeSession({ request, response }: HttpContext) {
    const { sessionToken, firebaseToken } = request.only(['sessionToken', 'firebaseToken'])
  ...
  private renderRedirect(response: HttpContext['response'], deepLink: string) {
    return response
      .status(200)
      .type('text/html')
      .send(renderRedirectHtml(deepLink))
  }
}
...
async function verifyFirebaseToken(_token: string): Promise<{ uid: string }> {
  const auth = getAuth()
  const token = _token.startsWith('Bearer ') ? _token.slice(7) : _token
